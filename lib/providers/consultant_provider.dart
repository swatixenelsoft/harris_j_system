...
class GetConsultantState {
  ...
  GetConsultantState copyWith({
    ...
    return GetConsultantState(
      ...
      consultantTimesheetRemark: consultantTimesheetRemark ?? this.consultantTimesheetRemark,
...
print('consultantClaimSheetResponse $consultantClaimSheetResponse');
      ...
      String token, String month, String year) async
  {
    try {
      ...
      await apiService.consultantTimesheetRemark(token, month, year);
...
// print()
  ...
  Future<Map<String, dynamic>> addTimesheetData(
    ...
    state = state.copyWith(isLoading: true, error: null);
    try {
      final addTimesheetResponse = await apiService.addTimeSheetData(token,
          userId, type, clientId, clientName, status, record,corporateEmail,reportingManagerEmail,selectMonth,selectYear, certificate);

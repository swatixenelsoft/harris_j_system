...
class _TimeSheetScreenState extends ConsumerState<TimeSheetScreen> {
  ...
  Map<DateTime, CalendarData> parseTimelineData(
    ...
    if (apiResponse['success'] == true &&
      ...
      final List dataList = apiResponse['data'];
      ...
      for (var dataItem in dataList) {
        ...
        for (var dayData in days) {
          ...
          if (details.containsKey('leaveType') && details['leaveType'] != null) {
            ...
            if (isCustom || ((shortLeaveType == 'ML' || shortLeaveType == 'AL') && suffix != null)) {
            ...
            calendarData[dateKey] = CalendarData(
              widget: Text(
                ...
                style: GoogleFonts.montserrat(
                  ...
                  color: int.parse(workingHours) != 8?const Color(0xffFF1901): const Color(0xff000000),
                ),
              ),
              type: 'work',
            );
  ...
  Future<void> _saveTimesheet(GetConsultantState consultantState) async {
    try {
      ...
      String monthName = DateFormat.MMMM().format(DateTime(selectedYear, selectedMonth));
      ...
      final filtered = days.where((day) {
        ...
        return details != null &&
            (details.containsKey('workingHours') || details['leaveType'] == 'PH');
      }).map((day) {
        final details = day['details'];
        return {
          'date': details['date'],
          details.containsKey('workingHours') ? 'workingHours' : 'leaveType':
          details.containsKey('workingHours') ? details['workingHours'] : details['leaveType'],
      ...
      final response = await ref.read(consultantProvider.notifier).addTimesheetData(
        token!,
        userId.toString(),
        type,
        clientId,
  ...
  Future<void> _submitTimesheet(GetConsultantState consultantState) async {
    if(_corporateEmail.text.isEmpty || _reportingManagerEmail.text.isEmpty){
      ToastHelper.showError(context, 'Please add corporate email and reporting manager email');
    }
    else
    try {
      // Start loader
    ...
    return Row(
      children: [
        ...
        CustomButton(
          ...
          borderRadius: 8,
          onPressed: () {
            _saveTimesheet(consultantState);
          },
        ...
        CustomButton(
          ...
          borderRadius: 8,
          onPressed: () {
            _submitTimesheet(consultantState);
          },
        ),
        const SizedBox(width: 7),
        GestureDetector(
  ...
  Widget _buildManagerInfo() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 10),
        CustomTextField(hintText: "Enter your corporate email-id",label:'Corporate email-id' , controller: _corporateEmail),
        const SizedBox(height: 10),
        CustomTextField(hintText: "Enter your reporting manager email-id", label:'Reporting manager email-id' ,controller: _reportingManagerEmail),
      ],
    );
  }

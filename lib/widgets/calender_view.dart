...
class _CalendarScreenState extends ConsumerState<CalendarScreen> {
  ...
  void _onPageChanged(int index) {
    setState(() {
      ...
      print(
          'Page index: $index, monthDifference: $monthDifference, _currentMonth: $_currentMonth');

      // widget.onMonthChanged?.call(_currentMonth.month, _currentMonth.year);
    });
    // Call the callback after this frame completes to avoid interfering with PageView
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isFromClaimScreen && !widget.isFromHrScreen) {
      print('ififif${widget.isFromClaimScreen && !widget.isFromHrScreen}');
      if (weeksInMonth == 6) {
      ...
      print('elelelele${widget.isFromClaimScreen && !widget.isFromHrScreen}');
      if (weeksInMonth == 6) {
        calculatedHeight += 35.0; // Add height for extra row (7 items)
      } else if (weeksInMonth == 5) {
        calculatedHeight += 30.0; // Slightly less if only 5 weeks
      }
    }
  ...
  Widget _buildMonthlyCalender(List<DateTime> monthDays) {
    ...
    int firstWeekday = firstDay.weekday % 7; // Make Sunday = 0, Monday = 1, ..., Saturday = 6

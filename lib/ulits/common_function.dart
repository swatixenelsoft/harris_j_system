...
class CommonFunction{
  ...
  Future<String?> selectDate(BuildContext context, TextEditingController controller) async {
    final DateTime now = DateTime.now();
    ...
    if (pickedDate != null) {
      final formattedDate = DateFormat('dd/MM/yyyy').format(pickedDate.toLocal());
          ...
          if (compressedFile != null) {
            ...
            if (size > 1024 * 1024) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Compressed file size still exceeds 1MB.")),
              );
              return null;
            }
          } else {
  ...
  void showPopupBelowWidget({
    ...
    final double leftPosition = leftPercent != null
        ? screenWidth * leftPercent
        : position.dx;
